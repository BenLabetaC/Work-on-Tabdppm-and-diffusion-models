...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, num_classes, is_y_cond, rtdl_params, dim_t, target_variable=None):
        super().__init__()
        self.dim_t = dim_t
        self.num_classes = num_classes
        self.is_y_cond = is_y_cond
        self.target_variable = target_variable  # Vous pouvez le stocker même si vous ne l'utilisez pas

        rtdl_params['d_in'] = dim_t
        rtdl_params['d_out'] = d_in

        # Assurez-vous que d_layers est une liste
        if isinstance(rtdl_params['d_layers'], int):
            rtdl_params['d_layers'] = [rtdl_params['d_layers']] * 2

        self.mlp = MLP.make_baseline(**rtdl_params)

        if self.num_classes > 0 and is_y_cond:
            self.proj_y = nn.Linear(1, dim_t)
        else:
            self.proj_y = None
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.is_y_cond and y is not None:
            if self.proj_y is not None:
                y = y.float().view(-1, 1)  # Redimensionner y pour qu'il soit compatible avec self.weight
                y = self.proj_y(y)  # Project y to dim_t dimensions
            emb += y
        x = self.proj(x) + emb
        return self.mlp(x)
