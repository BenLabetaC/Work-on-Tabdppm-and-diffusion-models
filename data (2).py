...
def prepare_fast_dataloader(dataset, split, batch_size):
    X_num = dataset.X_num[split]
    X_cat = dataset.X_cat[split]
    
    if X_cat.size > 0:  # Vérifiez si X_cat n'est pas vide
        if len(X_cat.shape) == 1:
            X_cat = X_cat.reshape(-1, 1)  # Reshape pour avoir deux dimensions
        X = torch.from_numpy(np.concatenate([X_num, X_cat], axis=1)).float()
    else:
        X = torch.from_numpy(X_num).float()  # Utilisez uniquement X_num si X_cat est vide
    
    y = torch.from_numpy(dataset.y[split]).long()
